...
public class DatabaseService implements IDatabaseService {
    ...
    public Set<Fognode> getChildren(){
        String jsonString = redisService.getValue(RedisKeys.CHILDREN.getKey());
        Set<Fognode> children = JSONHelper.toCollection(jsonString, HashSet.class, Fognode.class);
        return children==null ? new HashSet<Fognode>() : children;
    }
    ...
    public Set<Fognode> getParents(){
        String jsonString = redisService.getValue(RedisKeys.PARENTS.getKey());
        Set<Fognode> parents = JSONHelper.toCollection(jsonString, HashSet.class, Fognode.class);
        return parents==null ? new HashSet<Fognode>() : parents;
    }
    ...
    public void setChildren(Set<Fognode> children){
        String jsonString = JSONHelper.toJsonString(children);
        redisService.setValue(RedisKeys.CHILDREN.getKey(), jsonString);
    }

    public void addChild(Fognode child){
        Set<Fognode> children = getChildren();
        if(children == null){
            children = new HashSet<Fognode>();
        }
        children.add(child);
        setChildren(children);
    }

    public void removeChild(Fognode child){
        Set<Fognode> children = getChildren();
    ...
    public void removeChildren(){
        setChildren(new HashSet<Fognode>());
    }
    ...
    public void setParents(Set<Fognode> parents){
        String jsonString = JSONHelper.toJsonString(parents);
        redisService.setValue(RedisKeys.PARENTS.getKey(), jsonString);
    }

    public void addParent(Fognode parent){
        Set<Fognode> parents = getParents();
        if(parents == null){
            parents = new HashSet<Fognode>();
        }
        parents.add(parent);
        setParents(parents);
    }

    public void removeParent(Fognode parent){
        Set<Fognode> parents = getParents();
    ...
    public void removeParents(){
        setParents(new HashSet<Fognode>());
    }
