...
public class ReasonerServiceManyColonies implements IReasonerService {
    ...
    public Message handleTaskRequests(Application application) {
        ...
        // yes, in this scenario it we do not have nested FNs, only one FN and the closest neighbor FN
        if(isRootFN()){
            ...
            try {
                ...
                // ---------------------------------------------------------------------------------------------------
                // if some requests couldnt be deployed in the fog the application is propagated to the neighbor FN
                // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                ...
                deploymentTimes.put(application, seconds);
                String timeStr = new DecimalFormat("#.##########").format(seconds);
                log.info("total service deployment time : " + timeStr + " Seconds for " + requestCount+ " services of app "+application.getId().substring(5));
...
//                            stopFailedApplicationServices(assignment);
    ...
    private ApplicationAssignment deployServices(Set<TaskRequest> requests, Set<Fogdevice> children){
        //Set<TaskRequest> requests = new HashSet<TaskRequest>(application.getRequests());
        // split in fog colony, neighbor colony and cloud tasks (due to different processor architecture (ARM, Intel))
        ...
        List<TaskRequest> openedAfterDeployment  = assignment.getOpenRequests();
        List<TaskRequest> opened = new ArrayList<>();
        for(TaskRequest request: openedAfterDeployment){
            if(request.isCloudTask()) {
                cloudRequests.add(request);
                fogrequestCount=fogrequestCount-1;
            } else {
                opened.add(request);
            }
        }
        assignment.setOpenRequests(opened);
    ...
    /**
     * Propagate the requests of the passed application to the neighbor FN.
     * @param application application to propagate
     * @return message indicating whether it worked
     */

    private Message propagateTaskRequestsToNeighborFN(Application application){
        List<ServiceData> data = new ArrayList<ServiceData>();
        ServiceData sd = new ServiceData();
        sd.setReasoningPurpose(true);
        sd.setKey("taskRequests");
        sd.setApplication(application);
        data.add(sd);
        propagatorService.propagateToNeighbor(data);
        return new Message(Constants.URL_PROPAGATE, true);
    }
    ...
    private void stopFailedApplicationServices(ApplicationAssignment assignment){
        log.info("-- Stopping Application "+assignment.getId()+" with "+ assignment.getAssignedTasks().size()+" services");
    ...
    public boolean isRootFN(){
        Fogdevice parent = dbService.getParent();
        return parent != null && parent.getPort() == Constants.PORT_FOG_CONTROLLER;
    }
    ...
    private void fogLandscapeMonitoring(){
        // make sure only the root fn does monitor
        if(!isRootFN() || isReplanning || applicationAssignments.size() < 1) return;
    ...
    private void lostDeviceReplanning(Fogdevice fd){
        ...
        int applicationDuration = 60*1000;
        if(newAssignments!=null) {
            final String assignmentId = newAssignments.getId();
            // add timertask to the map in order to be able to stop it after the execution
            TimerTask t = new TimerTask() {
                @Override
                public void run() {
...
//                            stopFailedApplicationServices(assignment);
    ...
    public void deviceOverloadedEvent(Fogdevice fd){
        ...
        int applicationDuration = 60*1000;
        if(applicationAssignment!=null) {
            final String assignmentId = applicationAssignment.getId();
            // add timertask to the map in order to be able to stop it after the execution
            TimerTask t = new TimerTask() {
                @Override
                public void run() {
...
//                            stopFailedApplicationServices(assignment);
    ...
    public String getEvaluationSummary(){
        ...
        int totalExecutedRequestCount=0;
        for(Map.Entry e : deploymentTimes.entrySet()){
            ...
            Application app = (Application)e.getKey();
            int reqCount = app.getRequests().size();
            totalExecutedRequestCount+=reqCount;
        ...
        if(totalRequestCount > 0) {
            durationPerRequest = sumDuration / totalExecutedRequestCount;
