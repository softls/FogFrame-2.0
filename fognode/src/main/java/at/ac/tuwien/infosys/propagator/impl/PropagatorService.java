...
public class PropagatorService implements IPropagatorService {
    ...
    public void propagateToNeighbor(List<ServiceData> o){
        ServiceData s = o.get(0);
        if(s != null && s.getSender() != null) {
            //log.info("Propagating from "+ s.getSender().getIp()+ ":"+s.getSender().getPort());
        }
        try {
            commService.sendToClosestNeighborFN(Constants.URL_PROPAGATE, HttpMethod.POST, new HttpEntity(o),
                    new ParameterizedTypeReference<Object>() {});
        }catch (Exception ex){
            log.warn("Data propagation failed. Either the closest neighbor isn't available or not correctly configured.");
        }
    ...
    public List<TaskAssignment> propagateTaskRequestsToNeighborFN(List<TaskRequest> requests) {
        List<TaskAssignment> list = new ArrayList<TaskAssignment>();
        try {
            list = commService.sendToClosestNeighborFN(Constants.URL_PROPAGATE_TASK_REQUESTS, HttpMethod.POST,
                    new HttpEntity(requests), new ParameterizedTypeReference<List<TaskAssignment>>() {});
        } catch(Exception ex){
            log.warn("Task request propagation failed. Either the parent isn't available or not correctly configured.");
        }
