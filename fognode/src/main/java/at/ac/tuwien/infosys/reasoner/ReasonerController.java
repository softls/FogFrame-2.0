...
//        if(minutes == -1) time = -1;
     ...
     * @param countT4 amount of services of type t4
     * @param minutes duration of the application
     * @param deadline deadline on application deployment time
     * @return
     */
    @RequestMapping(method = RequestMethod.POST, value="reasoner/test/{countT1}/{countT2}/{countT3}/{countT4}/{minutes}/{deadline}")
    public Message testTaskHandlingWithDeadlines(@PathVariable int countT1, @PathVariable int countT2,
                                             @PathVariable int countT3, @PathVariable int countT4,
                                             @PathVariable int minutes, @PathVariable int deadline){
        ...
        // t4 = cloud db service (only cloud)
        for(int i = 0; i < countT1; i++){
            TaskRequest t = new TaskRequest(Constants.IMG_TEMPHUM_KEY, "t1", true, false, 0, deadline, minutes);
            testRequests.add(t);
        }
        for(int i = 0; i < countT2; i++){
            TaskRequest t = new TaskRequest(Constants.IMG_BUSYBOX_KEY, "t2", false, false, 0, deadline, minutes);
            testRequests.add(t);
        }
        for(int i = 0; i < countT3; i++){
            TaskRequest t = new TaskRequest(Constants.IMG_BUSYBOX_KEY, "t3", false, false, 0, deadline, minutes);
            testRequests.add(t);
        }
        for(int i = 0; i < countT4; i++){
            TaskRequest t = new TaskRequest(Constants.IMG_CLOUDDB_KEY, "t4", false, true, 0, deadline, minutes);
            testRequests.add(t);
        }
